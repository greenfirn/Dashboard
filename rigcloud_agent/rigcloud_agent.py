...
async def handle_command(raw, mqtt):
    ...
    # Parse JSON but do NOT restrict keys or values
    try:
        data = json.loads(raw)
    except:
    ...
    try:
        ...
        log(f"Command executed by script → {cmd}")

        # Optional telemetry after command
        await publish_status(mqtt, "cmd-run")

    except Exception as e:
        log(f"Command script error: {e}")
